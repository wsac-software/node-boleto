...
Boleto.prototype._calculate = function () {
  ...
  if (this.bank.options.codigo == '756') {
    this['nosso_numero_dv'] = formatters.mod11Sicoob(this['nosso_numero'], this['agencia'], this['codigo_cedente'] + '' + this['codigo_cedente_dv'])
  } else {
...
Boleto.prototype.renderHTML = function (callback) {
  ...
  const layout = `/../assets/${renderOptions.layout || 'layout'}.ejs`
  ejs.renderFile(path.join(__dirname, layout), renderOptions, {
    cache: true
  }, function (err, html) {
    if (err) {
      throw new Error(err)
